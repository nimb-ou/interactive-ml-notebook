    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'Both roles matter simultaneously: without γ < 1 an infinite sum of positive rewards diverges and cannot be compared across policies, and separately, the choice of γ is a modelling decision about how patient the agent should be — near 1 for long horizons, near 0 for myopic ones.'
      },
      {
        ...
        answer: 1,
        why: 'The log-derivative trick, $\\nabla\\pi_\\theta(a) = \\pi_\\theta(a)\\nabla\\log\\pi_\\theta(a)$, is what lets $\\mathbb{E}_{a\\sim\\pi_\\theta}[\\nabla\\log\\pi_\\theta(a)R(a)]$ be estimated from rollouts you can actually sample, rather than a gradient you cannot.'
      },
      {
        ...
        answer: 1,
        why: 'One-step episodes are why no bootstrapping or discounting is needed, and why the baseline (a learned critic in PPO, a sampled group\'s mean in GRPO) is the entire remaining variance story.'
      },
      {
        ...
        answer: 1,
        why: 'SARSA\'s on-policy update bootstraps off the action actually taken next, so it learns the value of its own (exploring) behaviour instead — which is why it is more cautious near a cliff edge.'
      }
    ],
    cards: [
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'The closed-form forward jump makes it a plain regression, which is why training is stable compared with GANs — no saddle point, no adversary.'
      },
      {
        ...
        answer: 1,
        why: 'Sums of independent Gaussians are Gaussian, so the whole multi-step noising chain reduces to one line relating $x_t$ directly to $x_0$ and a single noise draw.'
      },
      {
        ...
        answer: 1,
        why: 'An 8× spatial compression is 64× fewer values to process at every one of hundreds of sampling steps; decode to pixels once at the end. Conditioning enters via cross-attention, and DiT replaces the U-Net with a transformer.'
      },
      {
        ...
        answer: 1,
        why: 'Train the same network with and without the text conditioning, then push the prediction further in the direction the conditioning moved it than the conditioning alone suggests — trading diversity for prompt adherence.'
      }
    ],
    cards: [
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'The encoder can scatter training points into disconnected islands; a randomly sampled latent point from the gaps decodes to something the decoder never trained on.'
      },
      {
        ...
        answer: 1,
        why: 'Multiplying and dividing by $q_\\phi(z\\mid x)$ turns $\\log p(x)$ into $\\log\\mathbb{E}_q[\\cdot]$, and Jensen\'s inequality for concave $\\log$ gives $\\log\\mathbb{E}[X]\\ge\\mathbb{E}[\\log X]$ — the only approximation in the whole derivation.'
      },
      {
        ...
        answer: 1,
        why: '$z=\\mu+\\sigma\\odot\\epsilon$ with $\\epsilon$ drawn independently of $\\phi$ makes the path from $\\mu,\\sigma$ to $z$ differentiable — ordinary arithmetic instead of a sampling step.'
      },
      {
        q: 'Mode collapse is…',
        ...
        answer: 1,
        why: 'If one output already fools the current discriminator, gradient descent has no incentive to explore elsewhere — which is why the mitigations are game-theoretic (WGAN-GP, spectral norm, two-timescale updates) rather than ordinary regularisation.'
      }
    ],
    cards: [
...
${H.probe([
    ...
    quiz: [
      ...
      },
      {
        ...
        answer: 1,
        why: 'Global models on tabularised lags scale and exploit cross-series structure; backtest with rolling origin, never a random split.'
      },
      {
        ...
        answer: 1,
        why: 'This is §1.7\'s confounding problem in production form: the policy that generated the training data determines what can be learned from it.'
      }
    ],
    cards: [
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'PSI and IV bands are industry rules of thumb inherited from practice, not derived from any type-I error rate or hypothesis test — the same is true of the four-fifths rule in fairness (§2.19). The Hoeffding bound and the logistic gradient, by contrast, are genuinely derived results.'
      },
      {
        ...
        answer: 2,
        why: 'This is the specific, checkable version of "flag what is contested": the headline number survived scrutiny, a downstream detail did not, and conflating the two overstates your confidence in the detail.'
      },
      {
        ...
        answer: 1,
        why: 'Bandwidth arithmetic, cache formulas and attention mechanics survive for years; leaderboard positions do not survive a month. Knowing which layer a new fact belongs in tells you how much of your memory it deserves.'
      }
    ],
    cards: [
