...
<p>"Averages away" is doing real work in that sentence, and it is worth cashing out rather than trusting on faith — §1.8.1 already built the exact machinery needed to do it.</p>

${H.deriv('why repeated averaging collapses every node to the same value', [
      ['Strip $\\phi,\\psi$ down to the linear case the lab itself implements with its default mean aggregator: $h^{(l+1)} = P\\,h^{(l)}$, with $P=\\tfrac12 I+\\tfrac12\\hat A$ and $\\hat A$ the mean-over-neighbours operator.', 'This is not a simplification invented for the derivation — it is exactly the update rule the lab below runs when its aggregator is set to "mean", <code>nv[i] = .5*v[i] + .5*agg</code>, with the nonlinear $\\phi,\\psi$ set aside so the asymptotic behaviour is visible. Real GNNs add nonlinearities back in, but the phenomenon this derivation explains was first isolated, in the literature, in exactly this stripped-down linear case.'],
      ['$h^{(L)} = P^{L} h^{(0)}$', 'Applying the same fixed linear map $L$ times in a row — precisely the repeated-application argument §1.8.1 derived in general, $A^kv=\\lambda^kv$ for an eigenvector, extended to any starting vector by writing it as a combination of $P$\'s eigenvectors.'],
      ['Every row of $P$ sums to exactly 1, so $P\\mathbf 1=\\mathbf 1$: the constant vector is an eigenvector with eigenvalue $\\lambda_1=1$.', '$P$ is a convex combination of $I$ (row sums 1) and a mean-over-neighbours operator (also row sums 1, by definition of an average), so $P$ itself is row-stochastic — check it directly on the lab\'s own rule: $\\tfrac12(1)+\\tfrac12(1)=1$ for every row.'],
      ['For a connected graph, $\\lambda_1=1$ is the unique largest-magnitude eigenvalue, and every other $|\\lambda_i|<1$.', 'This is the Perron–Frobenius theorem applied to a non-negative, irreducible matrix — connectivity is exactly the condition that rules out the graph splitting into pieces that could each keep their own separate constant.'],
      ['So $h^{(L)} \\to c\\,\\mathbf 1$ as $L$ grows, for some constant $c$ set by the initial signal — every node converges to the identical value.', 'Exactly the §1.8.1 power-iteration argument: every eigen-component other than the top one decays like $\\lambda_i^{L}\\to0$, so only the $\\lambda_1=1$, constant-vector component survives, and "the arrow surrenders to the top eigenvector" becomes, on a graph, "every node surrenders to the same number."']
    ], 'This is the exact mechanism behind the gossip analogy\'s ending, not merely a metaphor for it — and §1.8.1\'s own governing ratio, $|\\lambda_2/\\lambda_1|$, carries over unchanged: a small, tightly-connected graph has a large spectral gap and over-smooths within a handful of hops, precisely what the lab shows happening by hop 5 or 6 on an eight-node graph; a large, sparse, path-like graph has $\\lambda_2$ much closer to 1 and can tolerate many more layers before collapsing. Depth safety in a GNN is not a fixed number of layers — it is a property of how quickly that specific graph mixes.')}
...
${H.practice(`<p>The habit worth taking away from all three items above generalises past this site, and it is cheaper than it sounds: checking a round, often-repeated ratio against one real example usually costs less time than it takes to quote it unchecked and hope nobody asks. Take the Chinchilla ratio itself. GPT-3, trained under the pre-Chinchilla scaling recommendation (§4.10), used roughly 175 billion parameters and was trained on roughly 300 billion tokens — a ratio near 1.7 tokens per parameter. At the ~20:1 ratio Chinchilla later argued was compute-optimal, a model that size would have wanted roughly $175\\text{B}\\times20\\approx3.5$ trillion tokens: more than ten times what it actually saw. That gap, computed from two public numbers in under a minute, is the entire empirical case for the Chinchilla paper stated as one division — GPT-3 was not undertrained by a rounding error, it was undertrained by an order of magnitude, and that is precisely the kind of check that takes longer to describe than to actually do.</p>`)}
