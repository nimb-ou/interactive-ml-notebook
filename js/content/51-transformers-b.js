...
${H.analogy(`<p>Clipping and the KL penalty are answering the same worry — "do not let the policy wander too far from something known to be reasonable" — on two different clocks, the way a dog on a leash is restrained by two different mechanisms at once. The leash's own length is like clipping: no single lunge can carry the dog more than a fixed distance from wherever the handler is standing <i>right now</i>, however hard it pulls in one go. But a handler who keeps taking one more step in the direction the dog is pulling, over and over, ends up somewhere completely different from where the walk started, even though no individual step ever broke the leash's length — which is exactly the failure clipping alone cannot prevent. The KL penalty is the handler's own memory of where the walk began: a standing cost, paid throughout, for how far the current position has drifted from the start, regardless of how many small individual steps it took to get there. A trust region bounds the step; a divergence penalty bounds the journey — RLHF needs both because "no single move was too big" and "we have not ended up somewhere unrecognisable" are genuinely different guarantees.</p>`)}
...
${H.intuition('The name is not decorative. In statistical physics, a system\'s state probabilities follow a Boltzmann distribution $\\propto\\exp(-E/T)$, where raising the temperature $T$ makes low- and high-energy states more nearly equally likely, and cooling the system toward absolute zero collapses it onto its single lowest-energy state. Softmax-with-temperature is the identical mathematical object with "energy" relabelled as "negative logit": a language-model logit is playing the role a physical system\'s energy plays, and dividing by $T$ before the exponential does exactly what cooling or heating a physical system does — turn $T$ down and the distribution collapses onto its single most likely token, exactly the way a physical system collapses onto its ground state at zero temperature; turn $T$ up and every option becomes closer to equally likely, exactly the way a hot system explores its available states more freely. Greedy decoding is not a separate idea from sampling with temperature — it is the $T\\to0$ limit of it.')}
<p><b>Top-k</b> keeps only the $k$ most likely tokens and renormalises among them — simple, but a fixed $k$ is the wrong tool whenever the shape of the distribution varies from position to position, which it constantly does: at a position where the model is highly confident, $k$ candidates may include tokens with vanishing probability that should never be sampled; at a position of genuine ambiguity, the same $k$ may exclude a perfectly reasonable continuation. <b>Top-p (nucleus) sampling</b>, introduced specifically to fix this, instead keeps the <i>smallest</i> set of tokens whose cumulative probability exceeds $p$ — a set that automatically shrinks to a handful of tokens when the model is confident and expands when it is genuinely uncertain, adapting to the distribution's actual shape rather than assuming a fixed count is ever right.</p>
