...
<p>Underneath all four sits one more question: even with the model split up, does the memory actually fit? <b>ZeRO / FSDP</b> answers it by attacking a specific redundancy in plain data parallelism — every GPU in that scheme holds a <i>full, independent copy</i> of the optimizer state, gradients and (in ZeRO-3/FSDP) even the parameters themselves, which is enormously wasteful when Adam's optimizer state alone needs roughly <b>12 bytes per parameter</b> of FP32 storage (a master FP32 weight copy plus two FP32 running moments, §4.11.2 explains why the FP32 copy exists), on top of the weights and gradients. ZeRO shards each of those tensors across the data-parallel group instead — every GPU permanently holds only its own $1/n$ slice — and gathers whatever full tensor is momentarily needed via targeted communication exactly when a computation requires it, then releases it again. That sharding, more than any other single trick, is what makes training a model whose optimizer state alone would not fit on one device possible at all.</p>

<p><b>What you are looking at.</b> Every memory consumer in a training step — parameters, gradients, optimizer state, activations — stacked as a bar and computed live from your chosen model size, GPU count, ZeRO stage and checkpointing setting, with a dashed line marking your card's actual memory limit.</p>
<p><b>What to do with it.</b> Start at ZeRO stage "none": watch the optimizer-state segment dominate the bar, larger than the weights themselves, and note whether the total clears your card's limit. Step through ZeRO-1, -2 and -3 and watch each successive stage shrink a different segment — optimizer state, then gradients, then parameters — as more of each tensor gets sharded across ranks rather than replicated.</p>
...
${H.probe([
      ...
      ['How much memory does Adam need?', '≈12 bytes per parameter of FP32 optimizer state — a master weight copy plus two running moments — on top of the weights and gradients, which is precisely what ZeRO/FSDP shards across data-parallel ranks rather than replicating on every GPU.'],
...
<p>The parameter saving is dramatic and easy to see directly: a $d\\times d$ matrix has $d^2$ parameters, while $B$ and $A$ together have $dr + rd = 2dr$. For $d=4096$ and $r=16$, that is $2\\times4096\\times16 \\approx 131{,}000$ against $4096^2 \\approx 16.8$ million — under 1% of the original matrix's parameter count, for a single target matrix, and the full-model figure worked out below lands in the same range once every targeted matrix in every layer is counted. Because $W_0$ never changes, there is no need to hold Adam's expensive optimizer state (§4.11's ≈12 bytes/parameter) for anything but $B$ and $A$ — the overwhelming majority of the model's parameters need no gradient and no optimizer state at all, which is where essentially all of the memory saving in the worked example below actually comes from. You train roughly 0.5% of the parameters, checkpoints are tens of megabytes rather than tens of gigabytes, and because $B$ and $A$ can be added to or removed from $W_0$ independently, an adapter can be hot-swapped at serve time — load a different tiny $B$, $A$ pair for a different customer or task without touching the multi-gigabyte base model at all.</p>

<h2><span class="sn">4.13.2</span> QLoRA</h2>
<p>LoRA already avoids optimizer state for the frozen base, but the base weights themselves still have to sit resident in memory at full or near-full precision to be read during the forward and backward pass. <b>QLoRA</b> shrinks that too, by keeping the frozen base compressed to 4-bit <b>NF4</b> — "4-bit NormalFloat", a quantization scheme whose representable values are spaced to match the roughly Gaussian distribution neural network weights actually follow, rather than spaced uniformly across the numeric range the way a naive 4-bit format would be, which wastes far fewer of its limited 16 representable levels on weight magnitudes that almost never occur. <b>Double quantization</b> squeezes a further small saving out of the scheme by quantizing the quantization constants themselves — the per-block scaling factors NF4 needs are, in aggregate, a real amount of memory at billions of parameters, and compressing them too is nearly free additional saving. <b>Paged optimizers</b> borrow the operating system's virtual-memory trick directly: when GPU memory comes briefly under pressure — a longer-than-usual sequence in the current batch, say — optimizer state pages out to ordinary CPU memory rather than crashing the run, and pages back in once space frees up. Throughout all of this, gradients still flow <i>through</i> the frozen quantized weights during backpropagation — the base is dequantized on the fly for each matrix multiply, and the chain rule passes a gradient back through that computation exactly as it would through any other layer — but nothing ever accumulates an update <i>into</i> the base weights; only $B$ and $A$, kept in ordinary BF16, are ever modified. That combination is what makes a 70B fine-tune fit on a single large GPU rather than a rack of them.</p>
...
<p><b>What you are looking at.</b> Every memory consumer for a LoRA fine-tune — frozen base, adapter weights, adapter optimizer state, adapter gradients, activations — stacked as a bar and computed live from the rank, targets and base precision you choose, with the full-fine-tune comparison figure printed alongside for scale.</p>
...
<p><b>The thing genuinely worth noticing.</b> Drag the base precision from 4-bit up to BF16 and watch the frozen-base bar alone grow past the entire PEFT total you started with — the base weights, even <i>frozen</i> and requiring no gradient, are still the single largest line item in the whole budget, which is exactly why QLoRA's quantization of the base, not just LoRA's low-rank adapter, is what makes a 70B-class fine-tune fit on one consumer-accessible card.</p>
...
${H.probe([
      ...
      ['The QLoRA trick?', 'Keep the frozen base compressed to 4-bit NF4 (spaced to match weights\' roughly Gaussian distribution) with double-quantized scaling constants; train only the BF16 adapter. Gradients flow through the dequantized base during backprop but never update it — only B and A ever change.'],
...
<p>Every one of these methods is ultimately pulling the same lever §4.7's roofline made explicit: decode is memory-bandwidth-bound, so fewer bytes read per weight directly cuts the dominant cost — quantizing a 70B model's weights from BF16 to INT4 shrinks the 42 ms per-step memory read (§4.7) roughly fourfold, which is a roughly fourfold ceiling lift on achievable decode throughput before anything else changes. <b>AWQ</b> earns its "activation-aware" name by first measuring which weight channels see the largest activation magnitudes in practice and protecting exactly those from aggressive rounding, rather than quantizing every weight with equal, uniform coarseness — a small amount of extra care that meaningfully closes the quality gap to full precision. Keep <b>weight</b> quantization conceptually distinct from <b>KV-cache</b> quantization: they attack the two separate memory consumers §4.7 itemised (parameters and the cache) and are two separate decisions with two separate costs, not one setting. And keep <i>post-training</i> quantization (GPTQ, AWQ, FP8 casting: minutes to hours, a small calibration set, where almost all production quantization actually happens) distinct from <i>quantization-aware training</i> (simulate low precision throughout training itself, so the weights adapt to it — better at aggressive sub-4-bit widths, far more expensive to run at all). If asked how to run a model in INT4 with minimal quality loss, the concrete answer is: AWQ, with a calibration set drawn from <i>your own</i> production distribution rather than a generic one, and an eval built specifically to catch the long-tail degradation quantization damage tends to show up in first.</p>
...
${H.probe([
      ...
      ['Which quantization in production?', 'AWQ INT4 or FP8 on GPU; GGUF only for local. Quantize the KV cache separately from the weights — they are two different memory consumers with two different costs.'],
