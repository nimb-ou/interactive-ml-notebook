...
<p><b>The thing genuinely worth noticing.</b> The compression ratios are enormous — 64× for product quantization — and free of any modelling cleverness; they come purely from the shortlist-then-rescore pattern doing the actual work. Set the index-choice readout to explain why: the advice text does not simply pick the most compressed option, because more compression always buys smaller memory at the cost of recall, and the honest answer at a million vectors is nearly always "HNSW in RAM, and stop optimising" — the interview-grade move is knowing the crossover point exists at all, not reaching for the fanciest index by default.</p>
...
<p><b>The thing genuinely worth noticing.</b> The readout below the wire trace reports round trips before useful work happens, whether sticky routing is required, and whether a gateway can cache the response — and on the old protocol, useful work cannot even start until two full round trips (initialize, then initialized) have completed, while on the new one the very first request is already doing something. That is not a minor latency saving; it is the difference between a protocol a load balancer has to be specially configured to understand and one that behaves like ordinary, cacheable, statelessly-routable HTTP.</p>
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'Every other change in the revision — per-request version and capabilities carried in metadata, the new server/discover RPC, routable headers, cacheable list results, Multi Round-Trip Requests replacing server-initiated calls — is a consequence of the same underlying decision: nothing about a request may depend on the server remembering anything from an earlier one. Once that decision is made, the initialize/initialized handshake and the Mcp-Session-Id header become not just unnecessary but actively wrong, because they are exactly the mechanism that made a server need to remember state, and everything else in the table is what had to be redesigned to replace the things sessions used to provide — mid-call user input, cacheable responses, routable requests — without bringing state back in through another door. A new authorization scheme is part of the release, but it is a smaller, separable change riding alongside the headline one, not the headline itself.'
      },
      {
