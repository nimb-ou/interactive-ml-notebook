...
<p>That warning is worth converting into arithmetic, because the arithmetic tells you not merely that the two stages are different but <i>which one to work on</i>, and it frequently disagrees with where a team's instincts point. Write $R$ for retrieval recall — the probability the evidence needed to answer a query makes it into the context at all — and $G$ for the probability the generator produces a correct answer <i>given</i> that the evidence is there. A query is answered correctly only if both happen, so</p>

$$P(\\text{correct}) = R \\times G$$

<p>Two consequences follow immediately, and the second is the one that changes behaviour. First, $R$ is a hard ceiling: since $G \\le 1$, end-to-end accuracy can never exceed $R$, no matter what model or prompt sits downstream. A system with recall of 0.7 cannot be made 80 per cent accurate by any amount of prompt engineering, and a team that does not know its $R$ can spend months discovering this empirically. Second — and this is the part the ceiling alone does not tell you — the <i>headroom</i> available from fixing each stage is wildly asymmetric, and which stage is worth more is a question with a numerical answer.</p>

${H.worked('which stage deserves the next month of work', `<p>Measure the two numbers separately, as the RAGAS decomposition above exists to let you do, and suppose they come back at $R = 0.70$ and $G = 0.90$. End-to-end accuracy is $0.70 \\times 0.90 = 63\\%$.</p>
<p>Now price each possible fix at its theoretical maximum — not what you expect to achieve, but what is even available to be won.</p>
<p><b>Perfect the retriever</b> ($R: 0.70 \\to 1.0$, generator untouched): $1.0 \\times 0.90 = 90\\%$. Gain: <mark>27 points</mark>.</p>
<p><b>Perfect the generator</b> ($G: 0.90 \\to 1.0$, retriever untouched): $0.70 \\times 1.0 = 70\\%$. Gain: <mark>7 points</mark>.</p>
<p>Retrieval is worth almost four times as much, and it would still be worth more even if you could only close half the retrieval gap against all of the generation gap. The general form is that the headroom in a stage is $(1 - \\text{that stage}) \\times \\text{the other stage}$, so the stage that is already worse has more room <i>and</i> gets its gains multiplied by the stage that is already good. Weakness compounds in your favour when you fix it.</p>
<p>The reason teams get this backwards is not innumeracy — it is that $G$ is far more <i>legible</i> than $R$. Generation failures are visible: you read a wrong answer, you can see exactly how it is wrong, and a prompt change produces a response you can inspect within seconds. Retrieval failures are invisible by construction, because the missing document is missing — the model receives a context that looks perfectly plausible, and answers from it fluently and wrongly, with nothing in the output marking the absence. The fix that feels productive and the fix that is worth four times more are, reliably, different fixes, and the only defence is measuring $R$ directly rather than inferring it from how the answers read.</p>`)}

${H.pitfall(`<p>Two caveats keep the factorisation honest. It assumes the generator's success is conditionally independent of <i>how</i> the evidence was retrieved, which fails in one direction that matters: dumping more chunks into the context to raise $R$ can lower $G$, because the generator now has to find the relevant passage among distractors. Recall and generation quality are not independent knobs past a certain $k$, which is exactly why "just retrieve top-50 instead of top-5" is not the free win the ceiling argument alone would suggest, and why a reranker — which raises the <i>precision</i> of what survives into the context without lowering recall of the candidate set — earns its latency.</p>
<p>The second is that $R$ must be measured against the evidence a query genuinely needs, not against a single gold document. Multi-hop questions need two or three chunks simultaneously, and if each is retrieved independently with probability $r$, then $R \\approx r^m$ for $m$ required pieces — the same compounding as §5.3's trajectory arithmetic, appearing here for the same reason. A retriever with a perfectly respectable $r = 0.85$ per fact delivers $R = 0.61$ on a three-hop question, which is usually the real explanation when a RAG system performs well on simple lookups and collapses on anything requiring synthesis.</p>`)}
...
<h3>The arithmetic of step count</h3>

<p>Everything above is qualitative, and the quantitative version is worth doing explicitly because it reorders what you would otherwise spend your engineering effort on. Suppose each step of a trajectory succeeds independently with probability $p$, and the task needs all $k$ of them to land. Independence is a genuine simplification and an optimistic one — the credit-assignment problem just described means real failures cluster, since one corrupted Observation makes the next step likelier to go wrong too — so treat what follows as a ceiling on how well a $k$-step agent can do, not a forecast.</p>

$$P(\\text{trajectory succeeds}) = p^k \\qquad\\Longleftrightarrow\\qquad \\log P = k \\log p$$

<p>The right-hand form is the one that carries the lesson. Log-success falls <i>linearly</i> in the number of steps, which means success itself falls geometrically, and a per-step reliability that sounds excellent in isolation stops sounding excellent almost immediately. At $p = 0.95$ — one step in twenty goes wrong, which most people would call a good tool-calling model — a ten-step task completes $0.95^{10} = 59.9\\%$ of the time, and a twenty-step task completes $0.95^{20} = 35.9\\%$ of the time. The model has not got worse between those two rows. Only the step count changed.</p>

${H.deriv('what per-step reliability a target trajectory success rate actually demands', [
      ['$p^k = T$', 'Set the trajectory success probability equal to whatever target $T$ the product requires — 90 per cent, say — and solve for the per-step reliability $p$ that delivers it.'],
      ['$p = T^{1/k}$', 'Take the $k$-th root of both sides. This is the specification a tool, a prompt or a model must actually meet, as opposed to the number anyone quotes when demoing a single call in isolation.'],
      ['$T = 0.9,\\ k = 5 \\ \\Rightarrow\\ p = 0.9^{1/5} = 0.979$', 'A five-step agent needs 97.9 per cent per-step reliability to be right nine times in ten. Demanding, but a well-scoped tool with a strict schema plausibly reaches it.'],
      ['$T = 0.9,\\ k = 20 \\ \\Rightarrow\\ p = 0.9^{1/20} = 0.9947$', 'A twenty-step agent needs 99.47 per cent — a failure rate of roughly one step in 190. Four times the steps demanded that the per-step <i>failure</i> rate fall by a factor of about four, from 2.1 per cent to 0.53 per cent.'],
      ['fix $T$, double $k$: $p_{\\text{new}} = T^{1/2k} = \\big(T^{1/k}\\big)^{1/2} = \\sqrt{p_{\\text{old}}}$', 'The general rule, and the one worth carrying: holding the success target fixed, doubling the length of a trajectory requires taking the <i>square root</i> of the per-step reliability. For $p$ near 1 that is very nearly halving the per-step failure rate — $\\sqrt{0.95} = 0.9747$, so 5 per cent failure becomes 2.5 per cent.']
    ], 'Read the last line as an engineering priority list rather than an identity. Halving the number of steps a task takes and halving the per-step failure rate buy <i>exactly the same amount</i> of trajectory reliability — but one of them is usually a scoping decision you can make this afternoon by merging two tools into one or precomputing a lookup the agent currently makes for itself, and the other is a research problem. This is the quantitative reason the strongest lever on agent reliability is almost always shortening the loop rather than improving the model inside it, and it is why a gate that lets a human resolve an ambiguity in one step is worth so much more than its own success rate suggests: it does not merely make one step reliable, it deletes the steps that would have followed a wrong guess.')}

${H.pitfall(`<p>The independence assumption above makes $p^k$ an <i>upper</i> bound, and it is worth being clear about which direction real systems deviate in, because the intuition cuts the wrong way at first. Correlated failures sound like they should be less damaging — surely if errors cluster, most trajectories are clean? But the clustering here runs forward in time, not across trajectories: one bad Observation raises the failure probability of every step that reads it, which is exactly the credit-assignment problem this subsection opened with. So a measured ten-step success rate below $p^{10}$ is not evidence that your per-step measurement was wrong. It is the expected signature of a loop where errors propagate rather than occurring afresh each step.</p>
<p>The practical tell is the shape of the failure distribution. If failures were genuinely independent, the step at which a trajectory first goes wrong would be roughly uniform across the trajectory. Plot where your failed runs actually break and the mass will usually sit early — not because early steps are intrinsically harder, but because a trajectory that survives its first few steps has demonstrated it is on a good path, while one that stumbles early was, from that point, reasoning from a false premise. That plot is worth more than an aggregate success rate, because it tells you whether to spend on making early steps more reliable or on catching errors mid-trajectory.</p>`)}
