...
<p>That parameter comparison is worth doing once at the scale of a real photograph rather than a 28-pixel test image, because the numbers stop being a ratio and start being an argument about what is possible at all.</p>

${H.worked('a dense layer against a convolutional one, on a 224×224 photograph', `<p>A standard ImageNet input is $224 \\times 224$ pixels in three colour channels, so $224 \\times 224 \\times 3 = 150{,}528$ numbers. Wire that to a modest 1,000-unit hidden layer the ordinary way, every input to every unit, and the weight matrix holds</p>
<p>$$150{,}528 \\times 1{,}000 = 150{,}528{,}000 \\text{ weights}$$</p>
<p>— just over 150 million parameters, in one layer, before the network has done anything else. Now the convolutional alternative: a $3\\times3$ kernel reading 3 input channels and producing 64 output channels needs $3 \\times 3 \\times 3 \\times 64 = 1{,}728$ weights, plus 64 biases, for <b>1,792 parameters</b> in total.</p>
<p>That is roughly <b>84,000 times fewer</b>, and the comparison understates the case, because the convolutional layer produces 64 full feature maps while the dense layer produces 1,000 lone numbers. <mark>The kernel's parameter count does not depend on the image's size at all</mark> — the same 1,792 weights process a 224-pixel image or a 4,000-pixel one, because the same kernel is reused at every position rather than each position getting its own weights.</p>`)}

${H.intuition(`<p>The saving is the visible consequence; the assumption underneath is the interesting part. Reusing one kernel everywhere is a claim about the world — that a feature worth detecting in the top-left corner is worth detecting in the bottom-right too, and detected the same way. That is <b>translation equivariance</b>, and for natural images it is very nearly true: an edge is an edge wherever it falls.</p>
<p>Seen that way, a convolution is a dense layer with most of its weights forced to zero and the survivors forced to be equal to each other. You are not discovering a cheaper layer, you are <i>constraining</i> a general one using something you already know about images. That framing explains the failure case too. Where the assumption is false — a structured form where position carries meaning, or tabular data whose columns have no spatial ordering at all — the constraint stops being free information and becomes a handicap, which is why convolutions never took over tabular problems (§2.7).</p>`)}

${H.pitfall(`<p>The equivariance is to <i>translation</i>, and to nothing else. Rotate the input, scale it, or flip it, and a convolutional network has no built-in reason to respond consistently — those invariances are not in the architecture and have to arrive through augmentation or be learned from data that happens to contain them.</p>
<p>This is a routine source of a model that scores well offline and disappoints in deployment: the training photographs were all upright because of how they were collected, the deployment photographs are not, and nothing in the architecture ever promised to cover the difference. §3.14 develops the general version of this, where the gap between the distribution you trained on and the one you serve is the thing that actually breaks models.</p>`)}
...
${H.intuition(`<p>The reason the spectral radius decides everything is that it is the only quantity that survives being multiplied by itself forty times. A gradient travelling back across $t$ steps picks up a factor that behaves like the recurrent matrix raised to the $t$-th power, and a repeated power has no middle setting: whatever is slightly less than one is driven to nothing, whatever is slightly more is driven to enormity.</p>
<p>Put the two cases side by side over a 40-step sentence, which is an ordinary length rather than an extreme one. At a radius of $0.9$, the surviving factor is $0.9^{40} \\approx 0.015$ — the gradient reaching the first word carries about one and a half per cent of its original magnitude, so the first word is, for learning purposes, not connected to the loss at all. At $1.1$, the factor is $1.1^{40} \\approx 45$, and the update explodes. Those two radii differ by about twenty per cent; the outcomes differ by a factor of three thousand. <mark>There is no setting of a plain recurrent weight matrix that both remembers forty steps back and stays numerically stable — the exponential is not a tuning problem, it is the shape of the function.</mark></p>`)}

${H.pitfall(`<p>The two halves of this failure look identical in a training log and need opposite responses, which is what makes them worth separating before you meet them.</p>
<p><b>Exploding</b> gradients announce themselves: the loss jumps to NaN, or spikes and never recovers, usually within a few steps. The fix is mechanical and reliable — clip the global gradient norm to a fixed ceiling and carry on. That works because the <i>direction</i> of an exploded gradient is still informative; only its length is wrong, and clipping rescales without rotating.</p>
<p><b>Vanishing</b> gradients announce nothing. Training proceeds, the loss falls, and nothing looks wrong — the model is simply learning short-range structure and silently ignoring anything far enough back. There is no threshold to clip and no spike to catch, which is why the fix had to be architectural rather than numerical, and why §3.8.3's gated cell state exists at all.</p>`)}

${H.history(`<p>The problem was diagnosed long before it was solved. Hochreiter identified the exponential decay in his 1991 diploma thesis, and Bengio, Simard and Frasconi gave the formal treatment in 1994, showing that the difficulty is intrinsic to the architecture rather than an artefact of poor optimisation — which is a much stronger and more discouraging claim.</p>
<p>The LSTM followed in 1997 from Hochreiter and Schmidhuber, and then waited. It needed roughly fifteen years of hardware and data before the sequence tasks it was built for were worth running at scale; the burst of practical results arrived around 2014 with sequence-to-sequence translation. The GRU that same year simplified the gating to two gates with broadly comparable results, and the argument over which is better was never settled decisively because the answer turned out not to matter for long. Attention (§4.1) removed the premise both were built on — that information from step 1 has to be carried forward through every intervening step — by giving every position a direct path to every other. The gating ideas did not disappear; §3.3.3's residual connection is the same trick applied to depth instead of time, and it is in every transformer.</p>`)}
...
<h2><span class="sn">3.9.4</span> Why a dot product means similarity at all</h2>

<p>Everything above has quietly assumed that a dot product measures how alike two things are, and that assumption deserves to be paid for rather than borrowed. §0.2 built the machinery: $a \\cdot b = \\|a\\|\\,\\|b\\|\\cos\\theta$, where $\\theta$ is the angle between the two arrows. Read that equation in the direction that matters here. The dot product is large when two vectors are long <i>and</i> point the same way; it is zero when they are perpendicular, whatever their lengths; and it is negative when they point in opposing directions. So a single number, cheap to compute, already carries the two facts you might care about — how strongly each vector asserts itself, and how much they agree.</p>

<p>That is why the training objective can shape a geometry at all. A model trained to give co-occurring words a high dot product has only two levers to pull: make the vectors longer, or rotate them toward each other. Lengthening helps every comparison that word takes part in, so it gets used mostly to encode something like general salience or frequency; rotation is the only lever that is <i>specific</i> to a pair. Over millions of updates the rotational pressure is what survives as structure, which is why direction ends up carrying the meaning and magnitude ends up carrying something closer to emphasis.</p>

${H.intuition(`<p>This also explains why the analogy arithmetic in §3.9.3 works at all, and why it is fragile. If a relation like "masculine to feminine" is encoded as roughly the same <i>displacement</i> wherever it appears, then subtracting one endpoint and adding another cancels the shared part and leaves the displacement — which is exactly what vector subtraction computes. The relation has to be consistent in direction across many pairs for that cancellation to land anywhere useful. Where a relation is irregular, there is no shared displacement to recover, and the arithmetic returns a point that is near nothing in particular.</p>`)}

<p>Once direction carries the meaning, the choice between cosine similarity and a raw dot product stops being a detail of configuration and becomes a question about what you want to rank by. <b>Cosine</b> divides the dot product by both lengths, so it reports the angle alone and ignores magnitude entirely. A raw <b>dot product</b> keeps the magnitudes in. Most of the time the two agree on the ordering, which is exactly what makes the disagreement dangerous — it is rare enough to go unnoticed and common enough to matter.</p>

${H.worked('a query where the two metrics rank differently', `<p>Take a query $q = (1, 0)$ and two candidates: $A = (3,\\; 0.3)$, which is long but slightly off-axis, and $B = (0.4,\\; 0.02)$, which is short but almost exactly aligned.</p>
<p>By dot product: $q \\cdot A = 3.000$ against $q \\cdot B = 0.400$, so <b>A wins comfortably</b>.</p>
<p>By cosine: $\\cos(q,A) = 3.000/(1 \\times 3.015) = 0.9950$ against $\\cos(q,B) = 0.400/(1 \\times 0.400) = 0.9988$, so <b>B wins</b>.</p>
<p>The two metrics return opposite orderings on the same two candidates. Nothing has gone wrong in either calculation — they are answering different questions. The dot product was asked "which candidate asserts the most agreement", and A is simply a bigger vector. Cosine was asked "which candidate points most nearly the same way", and B is better aligned. Note the resolution: normalise both candidates to unit length first and the dot product returns $0.9950$ for A, exactly its cosine. <mark>On normalised vectors the two metrics are the same function, which is why "normalise, then use dot product" is the standard way to get cosine behaviour at index speed.</mark></p>`)}

${H.pitfall(`<p>The failure this causes in production is quiet. An index built on inner-product search over unnormalised vectors will systematically favour documents whose embeddings happen to be long, and embedding norm often correlates with something incidental — document length, or how frequently the text's vocabulary appeared in training. The symptom is a retrieval system with a persistent taste for long documents that nobody can trace to a ranking rule, because there is no ranking rule to find: it is the metric doing what it was asked.</p>
<p>The rule that avoids it is narrow and worth stating exactly: <b>use the comparison the model was trained with.</b> A model trained with a cosine objective produces vectors whose magnitudes were never optimised for anything, so reading meaning into them is reading noise. §5.10 covers what this means once an approximate index is involved, where the choice is also baked into the index structure and cannot be changed after the fact.</p>`)}

${H.history(`<p>The dense-vector idea considerably predates neural networks. Latent semantic analysis in the late 1980s factorised a term-document matrix with the SVD of §1.8, on exactly the reasoning §3.9.1 gives — that a sparse high-dimensional count representation has far fewer real degrees of freedom than its width suggests.</p>
<p>What word2vec (2013) changed was cost. Rather than factorising a global matrix, it trained a shallow predictor on local windows with negative sampling, which made the whole thing streamable and turned embeddings from a batch computation into something you could run over a corpus of any size. GloVe (2014) then argued the two approaches were closer than they appeared, fitting a factorisation of log co-occurrence counts directly. The modern contextual embedding of §4.1 abandons the premise both shared — one fixed vector per word type — but keeps the geometry, which is why the similarity arithmetic in this section still describes how a transformer's hidden states behave.</p>`)}

<h2><span class="sn">3.9.5</span> What matters in practice</h2>
<ul>
<li><b>Cosine or dot product?</b> Cosine if you normalise (pure direction); dot product if magnitude carries meaning, such as term importance. Whichever the model was trained with is the one to use (§5.1), for the reason §3.9.4 works through.</li>
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'A transformer block is, structurally, attention and a feed-forward network each wrapped in a residual connection and a normalisation layer (§4.5 draws this with shapes) — remove either stabiliser and a stack of dozens of blocks stops training the way §3.3\'s vanishing-gradient argument predicts, because the residual path is what gives the gradient a route back to layer one with derivative exactly 1, and normalisation is what keeps activations in the range where that path stays useful. "Convolution and pooling" is tempting because both are genuinely important Part 3 ideas, but neither appears inside a transformer block at all: attention replaced convolution\'s local receptive field with an all-pairs comparison specifically to fix the parallelism and long-range-dependency limits §3.8 describes, so this option confuses "important elsewhere in Part 3" with "present here." "Dropout and BatchNorm" is the closer miss — dropout does appear in transformers but is not one of the two structural components every block is built from, and BatchNorm specifically is what LayerNorm/RMSNorm replaced (§3.6), precisely because BatchNorm\'s per-batch statistics behave badly on the variable-length sequences transformers process. The general principle is to distinguish an architecture\'s load-bearing structural components from techniques that are merely common or historically important, and §3.6\'s residual-plus-normalisation pairing is the one that survived unchanged into the modern default.'
      }
    ]
  });
})();
