...
${H.probe([
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'Minimising the per-leaf quadratic $Gw + \\frac12(H+\\lambda)w^2$ by setting its derivative to zero gives $w^* = -G/(H+\\lambda)$ — the one-dimensional case of Newton\'s step $-H^{-1}g$ from §1.9.3, with the curvature damped by λ before it is inverted. "The mean of the residuals" is the tempting answer because it is exactly correct for squared-error loss with $\\lambda=0$ — there, $g_i = F(x_i)-y_i$ and $h_i=1$, so $-G/H$ collapses to the mean residual — but it silently assumes both a specific loss and no regularisation, and the question is asking about the general XGBoost formula, which holds for any twice-differentiable loss. $G/(H-\\lambda)$ has the right shape and the wrong sign on λ: subtracting λ would make the leaf weight blow up as $H\\to\\lambda$, the opposite of what a damping term should do.'
      },
      {
    ...
    cards: [
      ...
      { q: 'Monotonic constraints — why say it', a: 'Domain agreement + validator story + free regularisation, at a cost of a point or two of AUC.' }
    ]
  });
...
<p>Equivalently, PCA is the SVD (§1.8.2) of the centred data matrix: the right singular vectors of $X$ after subtracting the column means are exactly the eigenvectors of $X^\\mathsf{T}X \\propto \\Sigma$, which is the identity §1.8.2 proved directly. This is the numerically stable way to compute PCA in practice — nobody forms $\\Sigma$ and eigendecomposes it by hand — and it is the reason "PCA" and "truncated SVD" are used almost interchangeably in the field.</p>

<h2><span class="sn">2.10.2</span> Why centring is not optional</h2>

<p>Everything above quietly assumed the covariance matrix $\\Sigma = \\frac1n\\sum_i (x_i-\\bar x)(x_i-\\bar x)^\\mathsf{T}$ was the object to eigendecompose — deviations from the mean, not the raw rows themselves. It is worth seeing exactly what goes wrong if you skip that subtraction and eigendecompose the raw second-moment matrix $M=\\frac1n\\sum_i x_ix_i^\\mathsf{T}$ instead, because "PCA needs centred data" is usually stated as a rule to remember rather than a fact you can watch break.</p>

${H.deriv('what the uncentered second-moment matrix actually contains', [
      ['$M = \\dfrac1n\\displaystyle\\sum_i x_ix_i^\\mathsf{T} = \\dfrac1n\\sum_i\\big[(x_i-\\bar x)+\\bar x\\big]\\big[(x_i-\\bar x)+\\bar x\\big]^\\mathsf{T}$', 'Add and subtract the mean inside each $x_i$ — a number that changes nothing, since it adds zero — to split every row into its deviation from the mean plus the mean itself.'],
      ['$= \\Sigma + \\dfrac1n\\displaystyle\\sum_i (x_i-\\bar x)\\bar x^\\mathsf{T} + \\dfrac1n\\sum_i \\bar x(x_i-\\bar x)^\\mathsf{T} + \\bar x\\bar x^\\mathsf{T}$', 'Expand the outer product of a sum exactly as you would $(p+q)(p+q)^\\mathsf{T}$: four terms, one of which is already $\\Sigma$ by its own definition.'],
      ['both cross terms vanish', '$\\frac1n\\sum_i(x_i-\\bar x)=\\bar x-\\bar x=0$ by the definition of the mean, so anything multiplied against it — the two middle terms above — is exactly zero, not merely small.'],
      ['$M = \\Sigma + \\bar x\\bar x^\\mathsf{T}$', 'What survives is the true covariance plus one extra rank-one term built entirely from the mean vector: the uncentered second-moment matrix is never wrong about $\\Sigma$, but it is never <i>only</i> $\\Sigma$ either — it always has this second object added in.']
    ], 'That extra term, $\\bar x\\bar x^\\mathsf{T}$, has exactly one non-zero eigenvalue, $\\|\\bar x\\|^2$, in the direction of $\\bar x$ itself — the direction from the origin to wherever your data happens to sit, which has nothing to do with how the data is shaped. Whenever the data sits far from the origin relative to its own spread, $\\|\\bar x\\|^2$ swamps every eigenvalue of $\\Sigma$, and the eigenvector M actually returns as "the direction of maximum variance" is simply the direction toward the mean.')}

${H.worked('the failure, with exact numbers', `<p>Suppose the true covariance is $\\Sigma=\\mathrm{diag}(4,\\,0.25)$ — feature 1 genuinely varies sixteen times more than feature 2 — but the data is not centred at the origin: its mean sits at $(0,\\,10)$. Centred PCA, exactly as built above, reports eigenvalues $4$ and $0.25$ with the top eigenvector $(1,0)$: feature 1, correctly, is where the real variation lives.</p>
<p>Now skip the centring step. By the identity just derived, $M=\\Sigma+\\bar x\\bar x^\\mathsf{T}=\\mathrm{diag}(4,\\,0.25)+\\mathrm{diag}(0,\\,100)=\\mathrm{diag}(4,\\,100.25)$. Eigendecompose <i>that</i>, and the top eigenvalue is $100.25$ with eigenvector $(0,1)$ — feature 2. The uncentered procedure reports, with complete numerical confidence, that feature 2 is the dominant direction of variance, when its true variance is the smaller of the two by a factor of sixteen. Nothing computed anything incorrectly; the matrix being decomposed was simply never the covariance matrix to begin with, once the mean was non-zero.</p>`)}

${H.pitfall('Skipping centring does not degrade PCA gracefully — it silently substitutes "the direction toward the mean" for "the direction of genuine spread", and the two are unrelated facts about the data. Whenever $\\|\\bar x\\|^2$ is large relative to the true eigenvalues of $\\Sigma$, which is the ordinary case for any feature that is not already mean-zero, the reported components are describing where the data sits, not how it is shaped.')}

<p>It should also, usually, be run on <i>scaled</i> features, unless every column genuinely shares the same units — otherwise "the direction of maximum variance" quietly becomes "the direction of whichever column happens to be measured in the smallest units", the identical failure shape as above with a rescaled column standing in for the mean-offset one, and exactly the mechanism §1.3's own correlation lab demonstrates when you push two features' standard deviations apart instead of their correlation.</p>

<h2><span class="sn">2.10.3</span> The reconstruction view, made numeric</h2>

<p>Every projection loses something, and it is worth having a formula for exactly how much rather than trusting the Eckart–Young citation on faith. Keeping the top $r$ components, a point reconstructs as $\\hat x = \\bar x + \\sum_{i=1}^{r} w_i\\big(w_i^\\mathsf{T}(x-\\bar x)\\big)$ — start at the mean, then add back the amount of movement along each kept axis that this particular point actually had.</p>

${H.deriv('average reconstruction error, in terms of the discarded eigenvalues alone', [
      ['$\\mathrm{avg}\\,\\|x-\\bar x\\|^2 = \\mathrm{trace}(\\Sigma) = \\sum_i \\lambda_i$', 'Total variance in the data, summed over every original axis, equals the trace of the covariance matrix — a standard fact about a symmetric matrix\'s eigenvalues (§1.8) — so it splits exactly into a sum over the eigenvalues, kept and discarded alike.'],
      ['write $t_i = w_i^\\mathsf{T}(x-\\bar x)$; then $\\|x-\\bar x\\|^2 = \\sum_i t_i^2$', 'The eigenvectors $w_i$ form an orthonormal basis (§1.8), so a vector\'s squared length is the sum of its squared coordinates in that basis — ordinary Pythagoras, applied in the rotated frame.'],
      ['$\\|x-\\hat x\\|^2 = \\Big\\|\\sum_{i>r} t_i w_i\\Big\\|^2 = \\sum_{i>r} t_i^2$', 'The reconstruction keeps exactly the first $r$ coordinates and drops the rest, so the error vector is built entirely from the discarded coordinates — again by Pythagoras, since the $w_i$ are mutually perpendicular.'],
      ['$\\mathrm{avg}[t_i^2] = \\mathrm{Var}(t_i) = \\lambda_i$', 'The projection onto eigenvector $i$ has variance exactly $\\lambda_i$ — this is what "$\\lambda_i$ is the variance captured along direction $i$" from §2.10.1 already meant.'],
      ['$\\mathrm{avg}\\,\\|x-\\hat x\\|^2 = \\displaystyle\\sum_{i>r}\\lambda_i$', 'Average the previous line over the whole dataset: the average squared reconstruction error is exactly the sum of the eigenvalues you chose not to keep — nothing more, nothing less.']
    ], 'Put the §2.10.1 numbers through it: $\\lambda_1=3,\\ \\lambda_2=1$. Keep only PC1 and the average reconstruction error is exactly $\\lambda_2=1$; total variance was $3+1=4$, so the fraction of variance <i>lost</i> is $1/4=25\\%$ — precisely $1$ minus the $75\\%$ explained-variance figure already quoted above. Explained-variance ratio and reconstruction error are not two separate things to track; one is a rescaling of the other, $\\text{fraction lost} = 1-\\text{explained variance ratio}$, for exactly the reason Eckart–Young says minimising one is maximising the other.')}
...
<h2><span class="sn">2.10.4</span> What PCA is not for</h2>
...
${H.pitfall('"PCA keeps the important features" is a category error, not merely an overstatement. Feature <i>selection</i> returns a subset of your original, named columns — you can still say "we dropped postcode and kept income". PCA returns a small number of <i>new</i> columns, each one a weighted sum of every original column at once: PC1 might be $0.31\\times\\text{income} + 0.28\\times\\text{spend} - 0.09\\times\\text{age} + \\cdots$ across all fifty original features, every one of them still present with some non-zero weight. There is no coefficient you can point to and say "this is the feature we removed" — you removed a <i>direction</i>, not a column, and every original column still contributes to whatever you kept. A regulator or a colleague asking "why does the model not use postcode" cannot be answered by pointing at a PCA loading; that question only has an answer in the original feature space, which PCA has already left behind by the time it hands back a score.')}
...
<h2><span class="sn">2.10.5</span> t-SNE and UMAP</h2>
