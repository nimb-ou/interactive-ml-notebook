...
${H.history(`<p>Before CLIP, the standard way to build a vision-language model was to run a pretrained object detector over an image — a network like Faster R-CNN, trained separately on a fixed, hand-labelled set of object categories — extract a handful of region features from whatever boxes it found, and feed those regions into a fusion model alongside text (LXMERT, ViLBERT and UNITER all worked this way). This has a specific, nameable failure: the vision half of the pipeline can never represent a concept the detector's fixed label set did not include, no matter how much text-side training data you throw at the problem, because information not extracted from the image in the first place cannot be recovered downstream. Radford et al.'s CLIP (2021) is the direct answer to that failure — throw the fixed label set away entirely, and instead learn what to look for purely from the free-form language paired with each image at web scale, so the vocabulary of visual concepts a CLIP-style model can represent is exactly as open as the captions it was trained on, not a curated list decided in advance by whoever built the detector.</p>`)}
...
${H.deriv('why the CLIP objective is an N-way classification problem, and why it needs a learned temperature', [
      ['$s_{ij} = \\hat u_i \\cdot \\hat v_j$ for L2-normalised image embedding $\\hat u_i$, caption embedding $\\hat v_j$', 'Cosine similarity between a unit-norm image vector and a unit-norm caption vector — normalising first means this dot product can only ever land in $[-1,1]$, regardless of how large the raw encoder outputs are.'],
      ['$P(\\text{caption }i\\mid\\text{image }i) = \\dfrac{\\exp(s_{ii}/\\tau)}{\\sum_{j=1}^N \\exp(s_{ij}/\\tau)}$', 'This is exactly §1.5\'s softmax turning a vector of scores into a distribution over $N$ classes — the "classes" here are simply "which of the $N$ captions in this batch matches this image", and cross-entropy against the identity matrix is asking for the true class, index $i$, to get all the probability mass.'],
      ['Without $\\tau$: sims cluster in a narrow band, e.g. $s = [0.32, 0.29, 0.25]\\Rightarrow$ softmax $\\approx[.344,.334,.321]$', 'Two unit vectors in a high-dimensional space are rarely more than moderately aligned or anti-aligned, so real cosine similarities across a batch typically span a narrow band well inside $[-1,1]$ — feeding that narrow band straight into a softmax with no rescaling produces an almost uniform distribution no matter how much better the true pairing actually is, because the gaps between scores are too small relative to 1 to move the exponential much.'],
      ['With learned $\\tau\\approx0.07$: same sims, scores become $s/\\tau=[4.6,4.1,3.6]\\Rightarrow$ softmax $\\approx[.495,.323,.182]$', 'Dividing by a small $\\tau$ multiplies every score by $1/\\tau\\approx14$, stretching the narrow band of raw similarities into a wide enough spread of logits that the softmax can actually commit to the correct pairing — identical in spirit to §4.3\'s $1/\\sqrt{d_k}$ attention scaling, which exists to stop dot products from saturating the softmax in the opposite direction.']
    ], 'CLIP is not "a similarity score with a training trick attached" — the identity-matrix cross-entropy loss is literally the same categorical cross-entropy §4.9.1 uses for next-token prediction, just with "which caption in this batch" standing in for "which token in the vocabulary". The learned temperature is not a cosmetic hyperparameter either: without it, L2-normalisation\'s own side effect — squeezing every possible score into $[-1,1]$ — would starve the softmax of enough spread to learn from, the identical problem large dot products solve for attention scores, approached from the opposite direction.')}
...
<p>CLIP solves representation — getting a useful vector out of an image — but it does not by itself solve <i>reasoning about</i> that image: a dual-encoder model can tell you how similar an image is to a caption, but it has no mechanism for answering an open-ended question about the image, generating free text conditioned on it, or attending back and forth between specific regions of the image and specific words of a question. Every row in the table below is a different answer to that specific gap, and the differences between them are really differences in <i>how much of the existing LLM's machinery gets reused as-is</i>. A dual encoder reuses none of it — there is no LLM in the loop at all, only a similarity score, which is exactly why it is fast enough to index but cannot reason. Cross-attention reuses the LLM's weights but bolts on a new mechanism (extra attention layers with their own parameters) for the text side to look at frozen visual features, paying an architecture-complexity and parameter-count cost to avoid disturbing the base language model. Early fusion reuses the LLM's forward pass completely unmodified — a projected image patch is, to every downstream layer, indistinguishable from a token embedding that happened to come from a word — which is precisely why it inherited the LLM's reasoning ability "for free" and became the dominant recipe: the cost it pays instead is context budget, spending thousands of the model's finite token slots on a single image, exactly the arithmetic the worked box below makes concrete.</p>
...
${H.pitfall('The instinct to route every document straight through a VLM because "it can read text in images" is worth resisting for anything text-heavy. A page of dense, well-scanned prose costs the same thousands of vision tokens whether the image is a hand-drawn diagram or a clean paragraph of typed text, because the vision encoder has no idea in advance which pixels are "just text" and which are genuinely visual content it needs its full resolution for — it tiles and encodes uniformly either way. A dedicated OCR pass on the same page routinely produces the same information in a few hundred text tokens rather than several thousand vision tokens, at a fraction of the cost and with no risk of the vision encoder mis-reading a character the way an image model occasionally does. The right default is to reserve vision tokens for pages where the visual layout itself carries information — figures, tables with meaningful spatial structure, handwriting, photographs — and to extract plain text first wherever the content is, in fact, just text that happens to be stored as an image.')}
    ...
    quiz: [
      {
        ...
        answer: 2,
        why: 'Divide each spatial dimension by the patch size and square the result: $(224/16)^2=14^2=196$, exactly §4.19.1\'s headline number, and because attention cost is quadratic in sequence length, this patch-size choice is the direct dial on both accuracy and compute for a vision transformer. "49" is the tempting wrong answer because it is the correct token count for the same image at a coarser 32×32 patch size, listed right alongside 196 in §4.19.1\'s design-point table — an easy mix-up if you recall the table\'s shape but not which patch size goes with which count. "768" is the embedding dimension a patch gets projected into, not a token count at all, and "16" just repeats the patch size back without squaring it. The general principle is that a ViT\'s only image-specific step is this patchify-and-project operation — everything after it is the identical transformer block from §4.5, which is why one architecture absorbed every modality once a way was found to turn it into vector-shaped pieces.'
      },
      {
        ...
        answer: 1,
        why: 'CLIP trains an image encoder and a text encoder contrastively so that a matched image-caption pair lands near each other in one shared embedding space, and once that space exists, classification requires no labelled training data for the new task at all: encode "a photo of a {class}" for every candidate class name as text, encode the image, and take the nearest — classification has become nearest-neighbour retrieval, exactly §4.19.2\'s table states. "It was trained on every class" is the tempting wrong answer because it would be a simple, sufficient explanation if true, and zero-shot performance can feel almost too good unless you have the actual mechanism in mind — but CLIP was never trained on any fixed class list at all, which is what makes the trick zero-shot rather than merely broad-coverage supervised classification. "It uses a softmax over classes" misdescribes the architecture, since there is no classification head anywhere in CLIP, and "it fine-tunes at inference" contradicts the entire point of encoding once and comparing distances. The general principle is that a shared embedding space, built once via contrastive training, converts an entire downstream task into a distance computation, which is why the same mechanism also gives cross-modal retrieval in both directions for free.'
      },
      {
        ...
        answer: 1,
        why: 'The contrastive InfoNCE objective is satisfied the moment matched pairs are pulled together and unmatched pairs pushed apart within a random batch, and a caption with its words reordered almost never appears as a hard negative in that batch, so nothing in the training signal ever forces the model to distinguish "a horse riding an astronaut" from its reverse — exactly the measurable, benchmark-documented (Winoground) weakness §4.19.2 names. "Slow inference" is the tempting wrong answer because CLIP is sometimes discussed alongside larger, slower multimodal systems, but CLIP\'s dual-encoder design is specifically prized for being fast: each side can be encoded once and indexed, the opposite of a speed problem. "Inability to handle colour" and "requiring labels" are not real properties of CLIP — CLIP is trained without labels in the classification sense, which is the whole point of the contrastive setup. The general principle is that a weakness this specific and reproducible traces directly back to what the objective does and does not reward, the same diagnostic move §4.9\'s discussion of pretraining objectives and §4.12.7\'s reward-hacking catalogue both make: look at what the loss actually measures, not at what you hoped it would produce.'
      },
      {
        ...
        answer: 1,
        why: 'An MLP projector maps visual features directly into the LLM\'s own token embedding space so the language model attends to image content as if it were simply more words in the sequence, using the identical attention mechanism throughout — the early-fusion design of LLaVA, Qwen-VL and GPT-4o-class models that §4.19.3\'s table marks as simplest, inheriting the LLM\'s reasoning for free at the cost of consuming context tokens. "A separate classifier whose label is inserted as text" is the tempting wrong answer because it sounds like a reasonable, modular bridge between two modalities, and something like it long predates transformers — but throwing away everything except a single predicted label discards almost all the visual information a capable VLM needs to answer open-ended questions about an image. "Fine-tuning the LLM on image bytes" ignores that raw pixels are not vector-shaped tokens without an encoder doing the work first, and "cross-attention only" names a real, competing architecture (Flamingo, Llama-3.2-vision) that keeps the LLM more intact but adds parameters rather than being the dominant current approach. The general principle is the same one opening this section: once a modality is turned into a sequence of vectors shaped like token embeddings, the transformer does not care where they came from, and early fusion is simply the most direct way to exploit that genericity.'
      }
    ],
    cards: [
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'A transformer performs a fixed amount of computation per generated token — L layers, once — so a problem whose answer requires more sequential steps than L cannot be solved in a single forward pass regardless of width; writing intermediate tokens is the workaround, because each token generated is another full forward pass and the written text is the scratchpad carrying state between them, converting a depth limit into a length budget exactly as §4.20.1 states. "The model explains its reasoning to itself" is the tempting wrong answer because it matches how chain of thought reads — a narrated, step-by-step account that sounds like introspection — but §4.20.1\'s key point is explicit that the written tokens are the computation, not a report about a hidden process happening elsewhere, which is also why a reasoning trace can be fluent and completely disconnected from the answer that follows it. "It reduces temperature" and "it retrieves better context" name mechanisms from entirely different sections with no role here. The general principle is a genuine complexity-theoretic one, not a metaphor: transformers with a polynomial-length scratchpad can express strictly more than transformers without one, which is why longer reasoning traces are a real, separate scaling axis from model size (§4.10).'
      },
      {
        ...
        answer: 1,
        why: '§4.20.2\'s derivation shows majority voting\'s accuracy climbing toward 1 as sample count grows only under Condorcet\'s jury theorem\'s assumption of independent errors; if the model instead makes the same mistake every time — a misread premise, a systematic misconception — every sample votes for that identical wrong answer, the effective number of independent votes collapses toward 1, and no amount of additional sampling helps, exactly the §2.16 ensemble-variance floor showing up in a different costume. "The answer space is large" is the tempting wrong answer because it sounds like it should make voting harder — more places for votes to scatter — but the derivation shows the opposite: a large answer space actually helps, because the correct answer then needs only a plurality rather than a majority, while wrong answers scattered thinly across many options are easy to outvote. "The temperature is too high" and "k is odd" are implementation details with no bearing on whether the underlying errors are independent. The general principle is that self-consistency is spending diversity, not merely quantity, as its resource — sampling more times only helps to the extent the additional samples can actually be wrong in different ways.'
      },
      {
        ...
        answer: 1,
        why: 'RLVR replaces the learned reward model RLHF depends on with a rule-based checker wherever one exists — did the computed answer match the known solution, did the generated tests pass — so the reward is not an approximation of ground truth standing in for it, it is ground truth mechanically checked, which is why there is nothing to reward-hack and training can run far longer before it degenerates, the property §4.20.2 and §4.12.5 both credit for underpinning current frontier reasoning models. "Larger" is the tempting wrong answer because RLVR-trained models genuinely do reach strong performance, and it is easy to mistake "the reward is more trustworthy" for "the reward is bigger in magnitude" — but nothing about RLVR changes the scale of the numeric reward, only where it comes from and whether it can be gamed. "Human-annotated" gets the mechanism backwards, since RLVR\'s appeal is removing the human or model-proxy step entirely for verifiable domains, and "given per token" describes a granularity question unrelated to what makes RLVR distinct. The general principle, §4.12.7\'s whole reward-hacking discussion, is that a learned reward model is always a proxy fit from finite data and optimising any proxy hard enough eventually finds where it disagrees with what it stood in for — a failure a mechanically-verified reward is structurally immune to.'
      },
      {
        ...
        answer: 1,
        why: '§4.20.3\'s flag is explicit: test-time scaling shows large, reproducible gains specifically on verifiable tasks — competition mathematics, code with tests, formal logic — where a program can check the final answer directly, while gains on open-ended tasks with no automatic checker are much smaller and harder to measure, precisely because the evaluation of those gains is itself a model rather than a mechanical check. "Chat helpfulness ratings" is the tempting wrong answer because it is exactly the kind of broad, headline-friendly metric a reader might expect strong evidence to come from, but helpfulness ratings depend on human or LLM-judge preference, which carries all of §4.16.2\'s documented judge biases and gives noisier signal than a program checking whether a proof or test suite passes. "Open-ended writing quality" and "summarisation" share the same fundamental measurement problem — no automatic ground truth exists to check against. The general principle is to be precise about which kind of task is meant when citing test-time compute results, since conflating verifiable and unverifiable domains is, as §4.20.3 states, the most common overclaim made about this technique.'
      }
    ],
    cards: [
...
${H.history('Constraining a generator\'s output to a formal grammar is not a language-model-era invention — speech recognisers have restricted their output to a JSGF grammar (a fixed set of command phrases a voice assistant can act on) since long before transformers existed, for exactly the same reason: a small, fixed language of valid outputs is worth enforcing mechanically rather than trusting the underlying model to stay inside it unprompted. What changed with Willard & Louf\'s Outlines (2023) and the wave of libraries that followed is scale and generality — compiling an arbitrary JSON Schema or context-free grammar into an automaton efficient enough to mask a 100,000-token vocabulary at every single decoding step of a frontier LLM, rather than a small, fixed command grammar known in advance.')}

<h2><span class="sn">4.21.2</span> How it is implemented</h2>
<p>"Compile the schema to an automaton" is doing a lot of work in one sentence, and it is worth seeing the compilation itself on an example small enough to draw by hand rather than taking it as a black box. Take the tiny schema the lab below actually uses: an object with two fixed keys, <code>name</code> (an enum of two values) then <code>age</code> (an integer). A <b>state</b> in the automaton is simply "how much of a valid string has been emitted so far, expressed as a position in the grammar" — after emitting <code>{"name":"Ada"</code>, the state is not the literal text, it is the fact "one key-value pair is complete, a comma or the closing brace must come next", and every different text that reaches that same fact is, for the automaton\'s purposes, the identical state. A <b>transition</b> is a token that is legal from a given state, labelled with which state it leads to; an <b>accepting state</b> is one where the string is allowed to end. Building the automaton means walking the schema once, enumerating every such state and, for each one, the set of tokens legal from it — exactly the sequence of fifteen states the lab steps through, from "the object must open" through to "generation complete". None of this depends on the model at all; it is pure formal-language compilation, done once per schema, and reused for every request that schema serves.</p>
${H.steps([
...
<p>That last row is worth dwelling on, because "tool calling" is routinely presented as if it were a separate capability an LLM has, rather than what it actually is: this section's exact mechanism, applied to a schema someone else — a developer integrating a weather API, a tool registry inside an agent framework (§5.3) — supplied at request time instead of one the model provider baked in. When an API call declares a function <code>get_weather(city: string, units: "celsius"|"fahrenheit")</code>, that declaration compiles to precisely the same kind of automaton a JSON Schema does: a state for "must choose one of the registered function names", a state for "now filling in the <code>city</code> argument", a state where <code>units</code> is constrained to exactly two legal token sequences and nothing else. The reason tool calls are reliably parseable in production — an agent that calls a real function with a malformed argument list is not a minor inconvenience, it is a runtime exception in someone else's system — is the identical guarantee §4.21.1 opened with: the argument list is valid by construction, not merely likely to be valid because the model was shown a few examples of the format. And the identical limit applies without modification: constraining the shape of a tool call guarantees the JSON parses and every argument has the right type, and does nothing whatsoever to guarantee <code>city: "Paris"</code> is actually the city the user meant, or that calling this tool was the right decision at all — that judgment is exactly as reliable as the model's own reasoning, schema or no schema.</p>
...
${H.deriv('why step-by-step masking is not the same as conditioning on validity', [
      ['Vocabulary $\\{A,B,C\\}$, two-token strings. Raw model: $P(\\text{1st}{=}A)=P(\\text{1st}{=}B)=0.5$; $P(\\text{2nd}{=}x)=1/3$ for every $x$, independent of the first token.', 'A deliberately simple, fully specified toy model — small enough to compute every one of its 6 possible two-token strings by hand.'],
      ['Valid strings (the "schema"): $\\{AC,\\ BC,\\ BB\\}$', 'A is followed by exactly one valid completion (C); B is followed by two (B or C) — a genuinely asymmetric branching structure, chosen deliberately so the two paths differ.'],
      ['True joint probability of each valid string: $P(AC){=}P(BC){=}P(BB)=0.5\\times\\tfrac13=\\tfrac16$. $P(\\text{valid})=\\tfrac16{+}\\tfrac16{+}\\tfrac16=\\tfrac12$. True conditional: $P(s\\mid\\text{valid})=\\tfrac{1/6}{1/2}=\\tfrac13$ for all three.', 'This is what "the distribution conditioned on validity" means precisely (§1.5\'s conditional probability): reject every invalid sample and renormalise what is left. All three valid strings had equal joint probability before conditioning, so they stay equal after it — genuine Bayesian conditioning cannot break a tie that was not already there.'],
      ['Step-by-step masked decoding instead: step 1 masks nothing (both A and B can still reach a valid completion), so $P(\\text{1st}{=}A)=P(\\text{1st}{=}B)=0.5$ unchanged. Step 2 given A: only C is legal, so $P(C\\mid A)=1$. Step 2 given B: B and C are both legal, renormalise $\\{1/3,1/3\\}\\to\\{0.5,0.5\\}$.', 'This is exactly what a real constrained decoder does at runtime: mask, renormalise the immediate next-token distribution, sample, repeat — with no lookahead beyond "does this token keep the string completable".'],
      ['Resulting joint: $P(AC)=0.5\\times1=0.5$, $\\ P(BB)=P(BC)=0.5\\times0.5=0.25$ each.', 'Multiply the (masked, renormalised) per-step probabilities along each path — ordinary chain-rule composition of a sequential sampling process.']
    ], 'The true conditional gives all three valid strings an equal one-third; step-by-step masking gives $AC$ a full one-half and starves $BB$ and $BC$ down to a quarter each. Nothing was implemented incorrectly — this is what correct, textbook masked decoding actually computes. The distortion has a precise cause: the A-branch has only one way to be valid, so all of its prior probability funnels undiluted into $AC$, while the B-branch\'s prior probability gets split across its two valid continuations. Step-by-step renormalisation has no way to look ahead and discount a branch for being narrower — true conditioning effectively does exactly that, because a branch with fewer valid completions contributes less total probability to the valid set in the first place. The gap is not a bug to fix; it is the mathematical price of computing a global conditional one greedy step at a time, and it is why, as the prose above states, heavily constrained generation can read as subtly, systematically skewed toward whichever branch happens to have the fewest valid continuations — even though every individual token sampled along the way was completely legal.')}

<h2><span class="sn">4.21.3</span> Design rules that matter more than the library</h2>
<p>It is worth being precise about <i>why</i> constrained decoding cannot, even in principle, extend to guaranteeing correct content, rather than treating it as an empirical limitation that a better grammar or a smarter library might someday close. A grammar, formally, defines a <b>language</b> — the set of token sequences a well-formed answer is allowed to be — and everything the automaton in §4.21.2 does is answer questions entirely internal to that set: is this prefix still extendable, is this string complete, is this next token one of the finitely many the grammar permits here. Truth is not a property of a string in isolation at all; it is a property of the relationship between a string and whatever fact in the world it claims to describe, and nothing about that relationship is encoded anywhere in a finite-state or pushdown automaton built purely from a schema's syntax. <code>{"total": 47.20}</code> and <code>{"total": 52.90}</code> are both members of the identical language — both perfectly well-typed instances of a schema requiring a numeric <code>total</code> field — and the automaton has no way to prefer one over the other, because "which one is the actual sum of the line items on this invoice" was never a question the grammar was built to answer. This is the same gap §4.16.4's account of hallucination describes from the training side — cross-entropy has no channel for checking a claim against reality — arrived at here from the decoding side: a constraint mechanism inherits exactly the same blind spot, because it operates one level of abstraction below where truth or falsity could even be represented.</p>
${H.checklist([
...
${H.probe([
    ...
    labs: {
      constrained: function (host) {
        ...
        Viz.note(host, 'Switch to <b>unconstrained</b> and watch the model happily assign 20% of its probability mass to <code>Hello</code> at a position where only <code>{</code> is legal. That is where your 3% JSON failure rate comes from. Count the steps marked <b>jump ahead</b>: for this rigid two-key schema, 13 of the 15 positions have exactly one legal token — only the two enum choices (the name, and the age) genuinely need the model at all — so a constrained decoder can skip roughly 87% of the forward passes on a schema this fixed, which is why it is often <i>faster</i> than unconstrained decoding, not slower. A schema with more open-ended fields will jump ahead less often than this one does; 13 of 15 is what <i>this</i> schema\'s rigidity buys, not a universal constant.');
      }
    },
    quiz: [
      {
        ...
        answer: 1,
        why: 'At every decoding step, the logits of every token that cannot continue a valid string under the schema\'s compiled automaton are set to $-\\infty$ before the softmax, removing those continuations from the probability distribution entirely rather than merely discouraging them — so invalid output becomes impossible to sample at any temperature, exactly §4.21.1\'s "invalid becomes impossible, not unlikely" framing. "Retrying until it parses" is the tempting wrong answer because it is the natural first thing anyone reaches for and does eventually produce valid output most of the time, but it is a probabilistic process with an unbounded tail — extra calls precisely when the model is already struggling — never a guarantee, the whole distinction the three-levels-of-guarantee table in §4.21.1 is built around. "Fine-tuning on the schema" raises natural compliance but still leaves some probability on invalid tokens unless combined with an actual constraint, and "post-processing the output" can only detect a failure after generation, not prevent it. The general principle is that guaranteeing a syntactic property of generated text is fundamentally a decoding-time intervention on which tokens are even sampleable, not something achievable purely through better prompting, training or after-the-fact repair.'
      },
      {
        ...
        answer: 1,
        why: 'Wherever the compiled automaton permits exactly one legal continuation — the colon after a key, a closing brace — that token can be emitted directly without calling the model at all, and for a sufficiently rigid schema this jump-ahead mechanism skips 30–50% of the forward passes entirely, exactly what §4.21.2\'s implementation steps describe and the lab\'s simulated JSON schema demonstrates. "The mask is cheaper than a softmax" is the tempting wrong answer because masking logits does sound like a lightweight operation, and it genuinely is cheap — but that saving alone is nowhere near enough to explain a real speedup, since the expensive part of a forward pass is the matrix multiplications through every layer, not the softmax at the end; the actual speedup comes from skipping entire forward passes. "It uses a smaller vocabulary" and "it stops earlier" both misidentify the mechanism — the vocabulary is unchanged, and generation still runs to the schema\'s natural end, just with fewer model calls along the way. The general principle is that a fully-determined position in a rigid schema carries zero information the model needs to supply, and recognising those positions in advance converts an unnecessary model call into a free, deterministic step.'
      },
      {
        ...
        answer: 1,
        why: 'Generation is causal — a token, once emitted, cannot be revised or informed by anything generated after it — so if the answer field comes before the reasoning field, the model has already committed to its answer before writing a single word of the reasoning meant to produce it, making that reasoning pure post-hoc narration; §4.21.3\'s design rule states this directly: put a reasoning field first if you need reasoning at all. "Last, after the answer" is the tempting wrong answer because it matches how humans often write — state the conclusion, then justify it — and can look tidier in a schema meant for downstream consumption, but that ordering is exactly backwards for a model whose only mechanism for "thinking" is generating tokens the causal attention mechanism can then condition on. "In a separate call" adds cost and complexity to solve a problem field ordering already solves for free, and "omitted" throws away the actual benefit of chain-of-thought-style reasoning entirely. The general principle connects directly to §4.20.1\'s account of why chain of thought works at all: the written tokens are the computation, so whatever computation you want performed has to be written before whatever depends on it.'
      },
      {
        q: 'Structured output guarantees…',
        options: ['correct content', 'valid shape only', 'both', 'neither'],
        answer: 1,
        why: 'A compiled grammar or JSON Schema constrains which token sequences are even syntactically well-formed, but nothing in that automaton checks whether a date is plausible, a total equals the sum of its line items, or a cited document actually supports the claim attributed to it — §4.21.3\'s pitfall states this directly: a model forced into a schema it does not understand fills it with confidently well-formed nonsense, worse than a parse error precisely because it passes every automated check. "Both" is the tempting wrong answer because constrained decoding feels like such a strong, total guarantee that it is easy to assume it extends to content as well as form — but shape and semantics are checked by entirely different mechanisms, and a schema\'s automaton has no way to represent "this fact is true." "Correct content" overclaims in the same direction, and "neither" undersells what constrained decoding actually delivers: a total, mechanical guarantee on syntactic validity. The general principle, restated across §4.16\'s hallucination discussion and here, is that structural fixes address structural problems — an automated check on shape cannot become an automated check on truth, so a semantic validation layer and an explicit "unknown" option remain necessary regardless of how tightly the schema is enforced.'
      }
    ],
    cards: [
    ...
    quiz: [
      {
        ...
        answer: 1,
        why: 'The accept/reject rule — accept a drafted token with probability $\\min(1,p(x_i)/q(x_i))$ and, on the first rejection, resample from the normalised residual $\\max(0,p-q)$ — is exactly the rejection-sampling construction that makes the composite process sample precisely from the target model\'s own distribution p, a theorem rather than an approximation, which is why §4.22.2 states the guarantee as provably exact. "Slightly, in exchange for speed" is the tempting wrong answer because nearly every other speedup technique in this part genuinely does trade some quality for latency — quantisation, aggressive truncation, a smaller model — so it is a reasonable prior to assume speculative decoding works the same way, but its entire design is built specifically to avoid that trade-off. "Only at high temperature" and "only for the bonus token" both imagine a partial guarantee that does not match the mechanism: the rejection-sampling construction holds at every position and every temperature setting. The general principle is that speculative decoding is a pure latency optimisation with zero quality cost, distinguishing it sharply from other levers in §4.14\'s serving toolkit that do trade something real for their speedup.'
      },
      {
        q: 'With $\\alpha = 0.8$ and $k = 4$, expected tokens per verification pass is…',
        options: ['4.0', '3.36', '2.4', '5.0'],
        answer: 1,
        why: 'Sum the geometric series $\\sum_{i=0}^{k}\\alpha^i=(1-\\alpha^{k+1})/(1-\\alpha)$ from §4.22.2\'s derivation: at $\\alpha=0.8,k=4$ that is $(1-0.8^5)/0.2=(1-0.328)/0.2\\approx3.36$, counting the free bonus token that comes for nothing when all four drafted tokens are accepted. "4.0" is the tempting wrong answer for anyone assuming every drafted token plus the bonus is accepted every time — the absolute ceiling of $k+1=5$ tokens, reached only in the limit $\\alpha\\to1$ — but with $\\alpha=0.8<1$ each successive token in the draft is discounted geometrically, so the true expectation sits below that ceiling. "2.4" undershoots the calculation, and "5.0" overshoots past even the theoretical maximum. The general principle is that this formula recurs across the course\'s worked numbers because it is short enough to derive live and sensitive enough to $\\alpha$ that a small drop in draft-model quality costs real speedup fast — exactly why production systems measure acceptance rather than assume a headline figure holds.'
      },
      {
        ...
        answer: 1,
        why: 'Every decode step already has to stream the entire model\'s weights (and the KV cache) from HBM to produce just one token\'s worth of arithmetic, so verifying k+1 positions in a single forward pass costs almost the same as verifying one — the weights are read once either way — which is exactly why speculation converts spare, otherwise-wasted FLOPs into fewer effective weight-reads per generated token, §4.22.1\'s whole "arithmetic intensity of decode is about 1 FLOP per byte against hardware that wants 200+" argument. "Compute-bound" is the tempting wrong answer because it correctly describes the other phase of a request — prefill, per §4.14.1\'s distinction — and it is easy to reach for the wrong half of that compute-vs-bandwidth pairing when asked about decode specifically. "Network-bound" and "limited by the softmax" both misattribute decode\'s bottleneck: single-request decode on one accelerator involves no network hop, and the softmax over the vocabulary is a tiny operation next to reading every weight. The general principle is that any technique amortising a memory-bound cost across more useful work per read — batching across requests, speculation across positions within one request — is exploiting the same roofline gap §4.7 first identified.'
      },
      {
        ...
        answer: 1,
        why: 'Large batches already amortise the weight read across many concurrent requests, which is precisely what makes decode compute-bound rather than memory-bound at that point, and once the GPU is compute-saturated a rejected draft token is not a free experiment any more — it is wasted computation that displaces real work another request needed, so speculation can actively reduce total throughput rather than merely fail to help, exactly §4.22.3\'s pitfall states. "Helps even more" is the tempting wrong answer because speculative decoding\'s benefit at low batch size is so large and consistent that it is natural to assume more scale only amplifies a good thing — but the entire mechanism depends on decode being memory-bandwidth-bound, and high batch size is specifically the regime where that assumption stops holding. "Has no effect" undersells the actual harm, since wasted draft computation genuinely competes for GPU cycles with useful work, and "becomes lossy" contradicts §4.22.2\'s exactness guarantee, which holds at every batch size regardless of throughput. The general principle is the same one running through §4.14\'s serving discussion: the right question is always whether you are optimising p50 latency for one user or tokens per second across many concurrent requests, because techniques that win at one end of that trade-off can lose at the other.'
      }
    ],
    cards: [
