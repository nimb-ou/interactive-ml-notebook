...
${H.intuition(`<p>The <b>cluster assumption</b> is doing all the work behind every method in this subsection: unlabelled data helps because it reveals where the data is <i>dense</i>, and the decision boundary is assumed to live in the sparse regions between clusters — push the boundary into a gap between two clouds of points and you have used geometry you got for free, no labels required. Consistency regularization is the cluster assumption applied as an objective directly: if $x$ and an augmented version of $x$ sit in the same cluster, as they should, the model's prediction should not change between them, and any change is treated as an error to correct.</p>
