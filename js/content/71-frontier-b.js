...
<h3>Sensitivity: how much one person can move the answer</h3>

<p>The definition above says what a mechanism must achieve but not how to build one, and every mechanism in the table below is calibrated to a single quantity that has to be computed before any noise can be chosen. That quantity is the <b>sensitivity</b> of the query: the most that one person's presence or absence can move its answer, taken as a worst case over every pair of datasets differing in one record.</p>

$$\\Delta_1 f \\;=\\; \\max_{D,\\,D'\\text{ differing in one record}} \\;\\big\\|f(D) - f(D')\\big\\|_1$$

<p>Read it aloud: over all the ways a single record could be added or removed, how far can the output travel? For a counting query — "how many people in this dataset have condition X" — the answer is exactly 1, because adding one person changes a count by one at most. For an average over $n$ people whose values lie in $[0, 1]$, it is $1/n$, because one person moves a mean by at most their own value divided by the count. For a <i>sum</i> of unbounded values it is infinite, which is why DP-SGD's clipping step exists at all: clipping each per-example gradient to norm $C$ is not a numerical convenience, it is the act of <i>manufacturing</i> a finite sensitivity where the raw query had none. A query with unbounded sensitivity cannot be privatised by adding noise, because no finite amount of noise is enough.</p>

<p>Once sensitivity is in hand, the Laplace mechanism's noise scale is not a tuning choice — it is forced, and the proof is short enough to do in full.</p>

${H.deriv('why Laplace noise at scale $\\Delta_1/\\varepsilon$ gives exactly $\\varepsilon$-DP', [
      ['$M(D) = f(D) + Z,\\qquad Z \\sim \\mathrm{Laplace}(b),\\quad p(z) = \\tfrac{1}{2b}e^{-|z|/b}$', 'The mechanism: compute the true answer, then add one draw of Laplace noise with scale $b$ still to be determined. The Laplace density is the same one met in §1.5.4 as a noise model — sharp peak at zero, exponential tails.'],
      ['$\\dfrac{p_D(y)}{p_{D\'}(y)} = \\dfrac{\\tfrac{1}{2b}\\exp(-|y-f(D)|/b)}{\\tfrac{1}{2b}\\exp(-|y-f(D\')|/b)} = \\exp\\!\\left(\\dfrac{|y-f(D\')| - |y-f(D)|}{b}\\right)$', 'Write the probability of landing on some output $y$ under each of the two neighbouring datasets and divide. The normalising constants $1/(2b)$ are identical on both sides and cancel; the exponents subtract.'],
      ['$\\big|\\,|y-f(D\')| - |y-f(D)|\\,\\big| \\;\\le\\; |f(D) - f(D\')| \\;\\le\\; \\Delta_1 f$', 'The reverse triangle inequality: two distances measured from the same point $y$ to two different points cannot differ by more than the distance between those two points. The second inequality is the definition of sensitivity — $\\Delta_1 f$ was defined as the worst case over exactly these pairs.'],
      ['$\\dfrac{p_D(y)}{p_{D\'}(y)} \\;\\le\\; \\exp\\!\\left(\\dfrac{\\Delta_1 f}{b}\\right)\\quad\\text{for every }y$', 'Substitute the bound into the exponent. This holds pointwise, at every possible output, which is what lets it integrate up to a statement about any output set $S$.'],
      ['$b = \\dfrac{\\Delta_1 f}{\\varepsilon} \\;\\Longrightarrow\\; \\dfrac{p_D(y)}{p_{D\'}(y)} \\le e^{\\varepsilon}$', 'Choose the scale that makes the bound equal the target. Integrating over any set $S$ gives $P(M(D)\\in S) \\le e^{\\varepsilon}P(M(D\')\\in S)$ — the definition, with $\\delta = 0$ exactly.']
    ], 'Notice what the final line does <i>not</i> contain: the dataset, the attacker, or any assumption about what either knows. The bound came from the shape of the Laplace density and one inequality about distances, so it holds for every output and every pair of neighbouring datasets simultaneously. That is the structural reason differential privacy survives auxiliary information when anonymisation does not — nothing in the proof ever needed to enumerate what the attacker might already have. It is also why the exponential appears in the definition at all: an exponential-tailed noise distribution is precisely the one whose likelihood ratio is bounded by a constant, and the definition was written around what this proof can deliver.')}

${H.worked('the same $\\varepsilon$ on a large group and a small one', `<p>Ask a counting query — how many people in the dataset have a given condition — so $\\Delta_1 f = 1$, and set $\\varepsilon = 1$. The Laplace scale is $b = 1/1 = 1$, and a Laplace variable with scale $b$ has standard deviation $b\\sqrt2 \\approx 1.41$.</p>
<p>On a subgroup of 5,000 people, a typical noise draw of $\\pm1.4$ against a count in the thousands is invisible — a relative error of about 0.03 per cent. The published number is, for every practical purpose, the true one.</p>
<p>On a subgroup of 3 people, that same $\\pm1.4$ is the entire signal. The published count could plausibly read 2, or 4, or 0, or 6. Nothing has been mis-implemented; the mechanism is doing exactly what it promised, which is to make the presence of any one person nearly undetectable — and in a group of three, "nearly undetectable" and "the answer is destroyed" are the same statement.</p>
<p>This is the whole reason differential privacy scales well and protects small groups badly, and why released DP statistics carry minimum-cell-size rules alongside their $\\varepsilon$. The noise is calibrated to one person's worst-case influence, which is an <i>absolute</i> quantity, while the useful signal grows with $n$. Sensitivity does not shrink as the group shrinks; only the thing you were trying to measure does.</p>`)}
