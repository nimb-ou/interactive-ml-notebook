...
${H.intuition(`<p>Both penalties are also, without changing a single number, exactly MAP estimation (§1.5) under a choice of prior on the weights. A zero-mean Gaussian prior on $w$ contributes a $-\\frac{1}{2\\tau^2}\\|w\\|_2^2$ term to the objective being maximised — negate it to get a loss, and it is precisely the ridge penalty, with $\\lambda=1/(2\\tau^2)$. A Laplace prior contributes $-\\frac{1}{b}\\|w\\|_1$, precisely the lasso penalty. So "add an L2 penalty" and "believe, before seeing any data, that large weights are a priori unlikely, in a bell-curve sort of way" are the same modelling choice stated in two vocabularies — and the fact that a Laplace prior concentrates more of its probability mass right at zero than a Gaussian does is the same fact, in the language of §1.5, that produces sparsity here.</p>`)}
